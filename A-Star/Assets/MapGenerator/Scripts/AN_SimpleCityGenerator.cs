...
public class AN_SimpleCityGenerator : MonoBehaviour
{
    ...
    private GameObject cityHolder;

    public void Generate() // use it in your games !
    {
        cityHolder = new GameObject("City");
            ...
            for (int y = 1; y < IntMap.GetLength(1) - 1; y++)
            {
                ...
                switch (IntMap[x, y])
                {
                    case (1): // 1x1
                        {
                            ...
                            GameObject building = Instantiate( List1x1[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, 90 * Random.Range(0,4), 0) );
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (21): // 1x2
                        {
                            selected = SelectPrefab(List1x2);
                            GameObject building = Instantiate(List1x2[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, 0, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (22): // 1x2 rotate -90
                        {
                            selected = SelectPrefab(List1x2);
                            GameObject building = Instantiate(List1x2[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, -90, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (31): // angle
                        {
                            ...
                            GameObject building = Instantiate(ListAngle[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, 0, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (32): // angle rotate -90
                        {
                            selected = SelectPrefab(ListAngle);
                            GameObject building = Instantiate(ListAngle[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, -90, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (41): // sqare
                        {
                            ...
                            GameObject building = Instantiate(ListSqare[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, 0, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    case (42): // sqare shift
                        {
                            selected = SelectPrefab(ListSqare);
                            GameObject building = Instantiate(ListSqare[selected].gameObject, new Vector3Int(x - MapLength / 2, 0, y - MapLength / 2) * SqareLength + CityCentre, Quaternion.Euler(0, 180, 0));
                            building.transform.parent = cityHolder.transform;
                            break;
                        }
                    default:
